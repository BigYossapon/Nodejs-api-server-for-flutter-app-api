...
app.put('/employees/edit/:ID', upload.single('Image_employee'), (req, res) => {
  ...
  if (req.file != null) {
    Image_employee = req.file.filename;
    sql = 'UPDATE employees SET Name = ? ,Phone = ? ,Address = ?, Position = ?, Mail = ? ,Image_employee = ?  WHERE ID = ?';
    db.query(sql, [Name, Phone, Address, Position, Mail, Image_employee, ID], (err, result) => {
      if (err) throw err;
      if (result.affectedRows === 0) {
        res.status(404).send('Employee not found');
    ...
    sql = 'UPDATE employees SET Name = ? ,Phone = ? ,Address = ?, Position = ?, Mail = ?   WHERE ID = ?';
    db.query(sql, [Name, Phone, Address, Position, Mail, ID], (err, result) => {
      if (err) throw err;
      if (result.affectedRows === 0) {
        res.status(404).send('Employee not found');
      } else {
